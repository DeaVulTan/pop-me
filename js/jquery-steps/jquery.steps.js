...
function paginationClickHandler(event)
{
    ...
    switch (href.substring(href.lastIndexOf("#")))
    {
        case "#finish":
			...
			var link = document.getElementById("select-options").elements[1].value;
			...
			$.ajax({
				...
				cache: false,
				beforeSend: function(){
					$('#order-status').html('<div class="alert alert-info fade in">Submitting order to our server..</div>');
				},
				success: function(data) {
					if(data) {
						$('#order-status').html('<div class="alert alert-danger fade in">'+data+'</div>');
					} else {
						$('#order-status').html('<div class="alert alert-success fade in">Order is successfuly submitted and it will be reviewed by our website administrators.</div>');
					}
				}	        
		   });
